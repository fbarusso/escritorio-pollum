...
export const convertMaxinutriTransportes = (
  fileType: string,
  fileContent: string
...
const payments = (fileContent: string): Result<string, string> => {
  try {
    ...
    return Failure(
      `Erro ao converter o arquivo de pagamentos para o Maxinutri Transportes: ${error}`
    );
  }
};
...
const receipts = (fileContent: string): Result<string, string> => {
  try {
    ...
    for (let i = 0; i < lines.length; i += 2) {
      ...
      outputValues5200[1] = "39";
      outputValues5200[2] = "1";
      outputValues5200[6] = inputValues5200[17] || "";
      ...
      outputValues5210[1] = inputValues5200[17] || "";
      outputValues5210[2] = "2008";
    ...
    return Failure(
      `Erro ao converter o arquivo de recebimentos para o Maxinutri Transportes: ${error}`
    );
  }
};
